...
ghsl-geoname-map: data-host ## Generate MaxMind geoname-id to GHSL urban-centre map and review report
	node scripts/build_ghsl_geoname_map.mjs --data-dir "$(MERIDIAN_DATA_DIR)" --output-dir "$(MERIDIAN_DATA_DIR)" --reports-dir "$(CURDIR)/reports/compatibility"
...
audit-maxmind-city: ghsl-geoname-map ## Audit MaxMind city names against IBGE and GHSL indexes
